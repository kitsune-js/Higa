...
import { Guild, GuildTemplate } from '../../structures/index.ts';
...
class GuildTemplateManager {
  ...
  public async getGuildTemplate(templateCode: string): Promise<GuildTemplate> {
    const res = await axiod.get<GuildTemplate>(
  ...
  public async createGuildTemplate(
    ...
    const res = await axiod.post<GuildTemplate>(
      `https://discord.com/api/v${this.version}/guilds/${guildId}/templates`,
      JSON.stringify(options),
      {
        headers: {
  ...
  public async syncGuildTemplate(
    ...
    const res = await axiod.put<GuildTemplate>(
...
export type { CreateGuildFromTemplateOptions, GuildTemplateOptions };
